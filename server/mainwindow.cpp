    ...
    else
    {
        ...
        if(connectedMode=="Tcp socket (QTcpServer)")
        {
            if(tcpSocket.at(indexOfSelection())->LastEditionMode=="toHex")
            {
                if(newMode=="Direct")
                {
                    tcpSocket[indexOfSelection()]->OutgoingData=QByteArray::fromHex(ui->textEditToClient->toPlainText().toLatin1());
            ...
            {
                if(newMode=="toHex")
                    ui->textEditToClient->setPlainText(ui->textEditToClient->toPlainText().toLatin1().toHex());
                else
                    ui->textEditToClient->setPlainText(QByteArray::fromHex(ui->textEditToClient->toPlainText().toLatin1()));
            }
        }
    }
}
...
bool MainWindow::ModeTxHaveChanged(QString newMode)
{
    ...
    if(LastEditionMode=="toHex")
    {
        if(newMode=="Direct")
        {
            OutgoingData=QByteArray::fromHex(ui->textEditToClient->toPlainText().toLatin1());
...
bool MainWindow::sendNewData()
{
    ...
    {
        block=ui->textEditToClient->toPlainText().toLatin1();
        qDebug() << "Data send:" << QString(block);
    }
    else if(ui->comboBoxTypeTx->currentText()=="toHex") //load the data in hexa mode
    {
        block=QByteArray::fromHex(ui->textEditToClient->toPlainText().toLatin1());
